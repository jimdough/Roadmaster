...
if (!function_exists('mh_scripts')) {
	function mh_scripts() {
		...
		wp_enqueue_style('mh-style', get_stylesheet_uri(), false, '1.8.6');
...
if (!function_exists('mh_excerpt')) {
	function mh_excerpt($excerpt_length = '175') {
		...
		if (!has_excerpt()) {
			...
			echo '<div class="mh-excerpt">' . esc_attr($excerpt) . ' <a href="' . esc_url(get_permalink($post->ID)) . '" title="' . get_the_title() . '">' . esc_attr($excerpt_more) . '</a></div>' . "\n";
		} else {
