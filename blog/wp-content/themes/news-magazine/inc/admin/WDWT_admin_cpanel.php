...
function wdwt_get_current_tab() {
    ...
    if ( isset( $_GET['tab'] ) ) {
        $current = $_GET['tab'];
        if($current != 'licensing' && $current != 'themes_support'){
          $current = 'licensing';
        }
    }
    else {
...
function WDWT_admin_scripts(){
  ...
  wp_localize_script( WDWT_VAR.'_admin_element_view_script', 'wdwt_slide_warning', __("You cannot delete the last slide! Try to turn off the slider", "news-magazine") );
