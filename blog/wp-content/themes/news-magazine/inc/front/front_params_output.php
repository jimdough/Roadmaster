 ...
 */
  ...
  */
  ...
  $color_scheme = esc_html( $this->get_param('[colors_active][active]') );
	...
	$text_headers_color = esc_html( $this->get_param('[colors_active][colors][text_headers_color][value]', array(), "#0480b4" ));
	$primary_text_color = esc_html( $this->get_param('[colors_active][colors][primary_text_color][value]', array(), "#838383" ));
	...
	$menu_links_color = esc_html( $this->get_param('[colors_active][colors][menu_links_color][value]', array(), "#838383" ));
	...
	$date_bg_color = esc_html( $this->get_param('[colors_active][colors][date_bg_color][value]', array(), "#484848" ));
	$lightbox_bg_color = esc_html( $this->get_param('[colors_active][colors][lightbox_bg_color][value]', array(), "#CCCCCC" ));
	...
	$lightbox_ctrl_btn_color = esc_html( $this->get_param('[colors_active][colors][lightbox_ctrl_btn_color][value]', array(), "#025a7e" ));
...
.top-nav-list li.current-menu-item> a, .top-nav-list li.current_page_item >a,#top-nav-list li.current-menu-item >a, #top-nav-list li.current_page_item >a{
	color: <?php echo $selected_menu_item_color; ?>;
}
...
.phone #top-nav-list > li ul li,.phone .top-nav-list> ul > li ul li{
	background-color:<?php echo '#'.$this->ligther($menu_color,10); ?>; 
}
...
for($i=0;$i<count($imgs_number);$i++){
  ...
  $image_textarea[$i] = isset($image_textarea[$i]) ? trim($image_textarea[$i]) : '';
    ...
    if( ($slider_show!="Hide Slider" && ((is_home() && $slider_show=="Only on Homepage") || $slider_show=="On all the pages and posts")) && count($imgs_url) && is_array($imgs_url)){  	?>
	<script>
	var data = [];   
	var event_stack = []; 
...
if(jQuery("#header_part #latest_posts ul li").length > active_post)
setInterval(function(){ 
	jQuery("#header_part #latest_posts ul li").removeClass("active_post");
	jQuery("#header_part #latest_posts ul li").eq(active_post).addClass("active_post");
	var gagas = active_post * 20;
	jQuery("#header_part #latest_posts ul li").eq(0).animate({ marginTop: -gagas+"px"},500);
    if (active_post === jQuery("#header_part #latest_posts ul li").length-1){
        active_post = 0;
    } else {
        active_post++;
    }	
